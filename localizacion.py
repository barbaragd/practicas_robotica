...
def localizacion(balizas, real, ideal, centro, radio, mostrar=0):
  ...
  # sensorial, dentro de una regian cuadrada de centro "centro" y lado "2*radio".
  x = 0
  y = 0
  imagen = []
  minError = 999999
  ...
  ideal.set(x, y, real.orientation)
...
localizacion(objetivos,real,ideal,centro,7,1)
...
for punto in objetivos:
  while distancia(tray_ideal[-1],punto) > EPSILON and len(tray_ideal) <= 1000:
    ...
    # # comprobar que estamos bien localizalizados
    prob = real.measurement_prob(ideal.sense(objetivos), objetivos)
    
    if prob > EPSILON:
      print "relocalizando"
      distancia_r_i = distancia(pose, real.pose())
      print "Distancia entre los robots: " + str(distancia_r_i)
      localizacion(objetivos, real, ideal, pose, 0.5)
